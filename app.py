...
with tab4:
    ...
    st.write(
        "- **銀灰色安全帽背面誤判修正**：透過失敗案例分析，發現早期版本容易將銀灰色安全帽背面高信心度誤判為未戴安全帽。最終版本針對此類易誤判情境進行資料補強與低品質樣本清理，提升模型於真實道路場景的泛化能力。"
    )
    st.write(
        "- **後處理 Bike Proximity Filter（雙 class 過濾）**：利用幾何演算法限制騎士偵測框（無論是否戴帽）與機車框之空間關係。此設計源自實際道路影片測試時發現路人也可能被誤判為 `rider_with_helmet`，因此將過濾範圍由單一類別擴展為雙類別，有效降低路人、靜態物體等 False Positive。"
    )

    st.markdown("### ⚠️ 專案挑戰：Data Leakage（資料洩漏）的發現與修正")
    st.warning("""
        **問題發現**：檢視 V2、V3 的資料處理流程時，發現驗證集存在 Data Leakage：V2 資料擴充時誤將 augmented 圖片一併納入資料集，導致同一張原圖的不同增強版本可能分別落入 Train 與 Valid，使當時的驗證結果偏高，無法作為模型比較依據。此問題未在 V3 修正，因此延續至後續版本。

        **修正方式**：撰寫自動化腳本，比對 V4 乾淨驗證集（54 張未經 augmentation 的原始影像）與 V1～V3 訓練集的檔名重疊率，確認資料重疊率隨版本迭代增加（V1：7.5% → V2：15.1% → V3：24.5%）。接著以這份乾淨驗證集重新評估 V1～V4 各版本模型；其中 V4 的重評結果（0.871）與原始記錄（0.869）幾乎一致，驗證了重評方法本身具有一致性。

        **結果**：重新建立乾淨驗證集後，得到較可靠的模型評估基準，修正後的 mAP50 演進（0.26 → 0.42 → 0.60 → 0.87）更能反映各次資料品質改善與模型迭代的實際成效。這次經歷也讓我體會到：驗證集指標未必能完全反映真實道路影片表現，因此最終模型選擇除了參考量化指標外，也會搭配固定道路測試影片進行驗證。
    """)
